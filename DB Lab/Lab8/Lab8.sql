...
--------------------------------------------------------------------------------------------------------------------
--Create a Trigger that automatically increments the 'articles_available'
--count in the 'inventory' table when a new deal is made.
Create Trigger inc_articles_available
On deals
For Insert
AS
Begin
...
Select @v_id= Inserted.v_id, @c_id=Inserted.c_id, @d_id=Inserted.d_id, @payment_mode=payment_mode, @payment_plan=payment_plan,
@paid=paid,@left_amount=left_amount,@date_deal=date_deal
From Inserted
print ('New Deal Sealed')

Update inventory
Set articles_available = articles_available + 1
Where modelno = (Select model_no From vehicle Where vehicle_id = @v_id)
And make = (Select make From vehicle Where vehicle_id = @v_id)
And company = (Select company From vehicle Where vehicle_id = @v_id)
end


--Disable the 'IncreaseInventoryArticles' trigger temporarily
disable trigger inc_articles_available on deals

--Enable the 'IncreaseInventoryArticles' trigger after disabling it.
enable trigger inc_articles_available on deals


--● View the status (enabled or disabled) of the 'IncreaseInventoryArticles' trigger.
select name, is_disabled
from sys.triggers
where name='inc_articles_available'

--● Remove the 'IncreaseInventoryArticles' trigger from the database.
drop trigger inc_articles_available
